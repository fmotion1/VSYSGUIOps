...
function Invoke-SpectreConsoleMultiselection {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
        [String[]]
    ...
    process {
        ...
        $msp = [Spectre.Console.MultiSelectionPrompt[string]]::new()
        $msp.HighlightStyle = $dStyle
        $msp.MoreChoicesText = "[grey](Move up and down to reveal more options)[/]"
        $msp.InstructionsText = "[grey](Press [cadetblue_1]<space>[/] to toggle an option., " +
        "[cadetblue_1]<enter>[/] to accept)[/]"

        foreach ($Choice in $Choices) {
            [void] $msp.AddChoice("[grey]$Choice[/]")
        }
        ...
        $FinalResult = @()
        foreach ($Result in $sResult) {
            $Result = $Result -replace '^\[grey\]', ''
